...
def run():
    ...
    with grpc.insecure_channel("localhost:8061") as channel:
        ...
        #registerModel
        stub.registerModel(model_handler_pb2.modelDefinition(
                needs_text = True,
                needs_image = False,
                can_text = True,
                can_image = False,
        ...
        #sendToModel
        send_to_model_response = stub.sendToModel(model_handler_pb2.taskRequest(
        ...
        #returnToTask
        return_to_task_response = stub.returnToTask(model_handler_pb2.modelAnswer(
                answer = "test answer",
                sessionID="123456789"
            )
        )
        print("After calling the returnToTask endpoint, model handler client received: ")
        print(return_to_task_response)

        #finishTask
        finish_task_response = stub.finishTask(model_handler_pb2.taskMetrics(
